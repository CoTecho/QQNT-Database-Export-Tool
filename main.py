...
from nt_msg.message import Message, Image
...
def export_chat_records():
    ...
    try:
        # 查询所有群聊消息，使用 peerUin (40027列) 匹配群号
        # 注：40030列有约0.16%的消息值为0，40027列更可靠
        messages_query = dbman.group_messages().filter_by(peerUin=target_group_uin).order_by(GroupMessage.msgSeq)
        ...
        for dbo in db_messages:
            ...
            try:
                ...
                # 统计图片数量
                image_count = sum(1 for elem in msg_obj.elements if isinstance(elem, Image))
